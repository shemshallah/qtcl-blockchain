...
@app.route('/api/quantum/lattice/status/full', methods=['GET'])
def lattice_status_full():
    """Get detailed lattice status with pretty formatting"""
    try:
        status = refresher.get_system_status()
        
        # Also trigger a console status report
        refresher.print_status_report()
...
@app.route('/api/quantum/lattice/force-status', methods=['POST'])
def force_status_report():
    """Force a status report to terminal/logs immediately"""
    try:
        refresher.print_status_report()
        return jsonify({
            'status': 'success',
...
def setup_routes(flask_app):
    ...
    # All your @app.route() decorated functions go here - just change @app to @flask_app
    # They will be defined as nested functions and registered once
    pass
