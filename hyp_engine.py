...
# ════════════════════════════════════════════════════════════════════════════
# §0b  SIG DICT NORMALIZATION — pre-verification unwrapper
#
# Handles every nesting/wrapping variant produced by the MCP sign pipeline:
#
#   Canonical (ideal):
#     {"R": {...}, "Z": {...}, "c_full": "hex...", "c_exp": N, ...}
#
#   MCP wrapper (outer envelope from _sign_message):
#     {"signature": json.dumps(canonical), "challenge": "hex...", "valid": True, ...}
#     → unwrap: json.parse(sig["signature"]) → canonical
#
#   Legacy / double-nested (old code path):
#     {"signature": '{"a":...,"b":...,"c":...,"d":...}', "challenge": "hex..."}
#     → sig["signature"] is the R matrix JSON, NOT the full canonical dict
#     → take legacy path: R = PSLMatrix.from_dict(sig["signature"]), Z=R placeholder
#
#   Mixed (partial unwrap):
#     {"signature": {"R":...,"Z":...,"c_full":...}, "challenge": ...}
#     → sig["signature"] is already a dict — use it directly as canonical
#
# After normalization the dict is guaranteed to contain EITHER:
#   • "R" (dict) + "Z" (dict) + "c_full" (str)  → canonical path in signature_from_dict
#   • "signature" (dict with a/b/c/d) + "challenge" (str)  → legacy path
# ════════════════════════════════════════════════════════════════════════════

def _normalize_sig_dict(sig: dict) -> dict:
    """
    Unwrap / normalize a signature dict so that signature_from_dict hits the
    canonical (R/Z/c_full) path rather than the legacy placeholder path.

    This is a pure normalization step — it does NOT modify cryptographic content.
    ...
    if "R" in sig and "Z" in sig and "c_full" in sig:
        return sig

    # Detect MCP wrapper envelope: has "signature" as a JSON string encoding
    # the full canonical dict (the double-wrap pattern).
    sig_field = sig.get("signature")
    if sig_field is not None:
        inner = None
        if isinstance(sig_field, str):
            try:
                inner = _json.loads(sig_field)
            except (ValueError, TypeError):
                inner = None
        elif isinstance(sig_field, dict):
            inner = sig_field

        if isinstance(inner, dict):
            # If inner has canonical fields → it's the real sig dict
            if "R" in inner and "Z" in inner and "c_full" in inner:
                # Merge: prefer inner for crypto fields, outer for metadata
                merged = dict(sig)           # start with outer (has challenge, public_key, etc.)
                merged.update(inner)         # override with inner canonical fields
                return merged

            # inner has R+Z but not c_full → partial canonical; promote what we have
            if "R" in inner and "Z" in inner:
                merged = dict(sig)
                merged.update(inner)
                # Promote challenge → c_full if missing
                if "c_full" not in merged and "challenge" in merged:
                    merged["c_full"] = merged["challenge"]
                return merged

            # inner looks like a matrix dict (a,b,c,d) → legacy R-matrix path:
            # leave sig as-is so signature_from_dict uses its legacy branch.
            if all(k in inner for k in ("a", "b", "c", "d")):
                # sig["signature"] is the R matrix JSON-string — convert to dict
                # so signature_from_dict doesn't need to json.loads it again.
                normalized = dict(sig)
                normalized["signature"] = inner
                return normalized

    # Promote challenge → c_full for engines that only set "challenge"
    if "c_full" not in sig and "challenge" in sig and ("R" in sig or "Z" in sig):
        normalized = dict(sig)
        normalized["c_full"] = sig["challenge"]
        return normalized
...
class HypGammaEngine:
    ...
    def verify_signature(self, message_hash: bytes, sig: Dict[str, str],
        ...
        try:
            ...
            # ── Normalize sig dict ─────────────────────────────────────────────
            # Unwrap any double-nested JSON that can accumulate when the MCP layer
            # json.dumps the canonical dict and the caller json.parses the outer
            # wrapper but not the inner 'signature' field.
            #
            # After normalization, sig MUST have either:
            #   Canonical path: R (dict), Z (dict), c_full (str)   ← preferred
            #   Legacy path:    signature (dict|str→dict), challenge (str)
            sig = _normalize_sig_dict(sig)

            if 'signature' not in sig and 'R' not in sig:
                raise ValueError("sig missing required keys")
            if 'R' not in sig and 'challenge' not in sig:
                raise ValueError("sig missing required keys")

            logger.debug(f"Verifying signature: {sig.get('challenge', sig.get('c_full', '?'))[:16]}...")
            ...
            with self._lock:
                ...
                logger.info(
                    ...
                    f"has_R={'R' in sig} has_Z={'Z' in sig} has_c_full={'c_full' in sig} "
