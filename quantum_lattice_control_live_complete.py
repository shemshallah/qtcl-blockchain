...
# ═════════════════════════════════════════════════════════════════════════════════════════
# INTEGRATED: Noise-Based Entanglement Validator (from noise_based_entanglement_validator.py)
# ═════════════════════════════════════════════════════════════════════════════════════════
# Three independent measurement protocols for tangible entanglement proof
# No new files - all integrated directly into this module

try:
    from globals import get_qrng, get_heartbeat, get_db_pool, get_globals, GLOBALS_STATE, PROFILER
    GLOBALS_AVAILABLE = True
except ImportError:
    GLOBALS_AVAILABLE = False
    logger.warning("[Validator] Globals not available - using fallback")


class PartitionEntropyMeasurer:
    """Measure entanglement entropy across bipartite systems (S_A)"""
    
    def __init__(self, n_qubits: int = 106496, partition_ratio: float = 0.5):
        self.n_qubits = n_qubits
        self.partition_point = int(n_qubits * partition_ratio)
        self.history = deque(maxlen=100)
        self.lock = threading.RLock()
    
    def compute_entropy(self, eigenvalues: np.ndarray) -> float:
        """Von Neumann entropy: S(ρ) = -Σ λᵢ log₂(λᵢ)"""
        evals = eigenvalues[eigenvalues > 1e-12]
        if len(evals) == 0:
            return 0.0
        evals = evals / np.sum(evals)
        entropy = -np.sum(evals * np.log2(evals + 1e-15))
        return float(entropy)
    
    def measure(self, rho: np.ndarray) -> Dict[str, float]:
        """Measure entanglement entropy of partition A"""
        with self.lock:
            try:
                evals_full = np.linalg.eigvalsh(rho)
                S_full = self.compute_entropy(evals_full)
                
                dim_total = rho.shape[0]
                dim_A = self.partition_point
                dim_B = dim_total - dim_A
                
                rho_reshaped = rho.reshape((dim_A, dim_B, dim_A, dim_B))
                rho_A = np.einsum('ikjk->ij', rho_reshaped)
                rho_A = (rho_A + rho_A.conj().T) / 2.0
                
                evals_A = np.linalg.eigvalsh(rho_A)
                S_A = self.compute_entropy(evals_A)
                
                result = {
                    'S_A': S_A,
                    'S_total': S_full,
                    'is_entangled': S_A > 0.5,
                    'timestamp': time.time()
                }
                self.history.append(result)
                return result
            except Exception as e:
                logger.debug(f"[PartitionEntropy] Error: {e}")
                return {'S_A': 0.0, 'S_total': 0.0, 'is_entangled': False}


class EntanglementWitness:
    """Witness observable for W-state detection (<W> < 0 = proven entanglement)"""
    ...
    def __init__(self, n_qubits: int = 5):
        ...
        self.history = deque(maxlen=100)
        self.lock = threading.RLock()
        self.w_state_projector = self._build_w_state_projector()
        self.witness = np.eye(self.dim) - self.w_state_projector
    
    def _build_w_state_projector(self) -> np.ndarray:
        """Build |W⟩⟨W| for n-qubit W-state"""
        basis_indices = [2**i for i in range(self.n_qubits)]
        w_vec = np.zeros(self.dim, dtype=complex)
        for idx in basis_indices:
            w_vec[idx] = 1.0 / np.sqrt(self.n_qubits)
        return np.outer(w_vec, w_vec.conj())
    
    def measure(self, rho: np.ndarray) -> Dict[str, Any]:
        """Measure witness and determine entanglement"""
        with self.lock:
            try:
                exp = np.real(np.trace(self.witness @ rho))
                result = {
                    'expectation': float(exp),
                    'is_entangled': exp < -1e-6,
                    'confidence': abs(exp),
                    'status': 'definitely_entangled' if exp < -1e-6 else 'likely_separable' if exp > 0.05 else 'inconclusive',
                    'timestamp': time.time()
                }
                self.history.append(result)
                return result
            except Exception as e:
                logger.debug(f"[Witness] Error: {e}")
                return {'expectation': 0.0, 'is_entangled': False, 'confidence': 0.0, 'status': 'error'}


class CascadingMeasurementAnalyzer:
    """Measure measurement-induced correlations (MI > 0.15 = entangled)"""
    
    def __init__(self, validator_qubits: int = 5, total_qubits: int = 106496, n_samples: int = 256):
        self.validator_qubits = validator_qubits
    ...
    def measure(self, rho: np.ndarray) -> Dict[str, Any]:
        """Compute measurement-induced correlation"""
        with self.lock:
            try:
                evals = np.linalg.eigvalsh(rho)
                evals = np.maximum(evals, 1e-15)
                evals = evals / np.sum(evals)
                S_AB = -np.sum(evals * np.log2(evals + 1e-15))
                cascade_strength = min(S_AB, 1.0)
                
                result = {
                    'cascade_correlation': float(cascade_strength),
                    'is_entangled': cascade_strength > 0.15,
                    'strength_level': 'STRONG' if cascade_strength > 0.4 else 'MODERATE' if cascade_strength > 0.25 else 'WEAK' if cascade_strength > 0.15 else 'NONE',
                    'timestamp': time.time()
                }
                self.history.append(result)
                return result
                ...
                return {'cascade_correlation': 0.0, 'is_entangled': False, 'strength_level': 'ERROR'}


class NoiseBasedEntanglementValidator:
    """Master validator orchestrating all three measurements"""
    
    def __init__(self):
        self.partition_measurer = PartitionEntropyMeasurer()
        self.witness = EntanglementWitness(n_qubits=5)
        self.cascade = CascadingMeasurementAnalyzer()
        self.composite_history = deque(maxlen=100)
        self.lock = threading.RLock()
        self.cycles_validated = 0
        logger.info("[NoiseBasedValidator] Initialized with 3 measurement protocols")
    
    def validate_cycle(self, rho: np.ndarray, cycle_num: int) -> Dict[str, Any]:
        """Run complete validation suite"""
        with self.lock:
            partition_result = self.partition_measurer.measure(rho)
            witness_result = self.witness.measure(rho)
            cascade_result = self.cascade.measure(rho)
            
            entanglement_votes = sum([
                partition_result.get('is_entangled', False),
                witness_result.get('is_entangled', False),
                cascade_result.get('is_entangled', False)
            ])
            
            composite = {
                'cycle': cycle_num,
                'S_A': partition_result.get('S_A', 0.0),
                'witness_exp': witness_result.get('expectation', 0.0),
                'cascade_corr': cascade_result.get('cascade_correlation', 0.0),
                'entanglement_votes': entanglement_votes,
                'is_entangled': entanglement_votes >= 2
            }
...
_ENTANGLEMENT_VALIDATOR = None

def get_validator():
    global _ENTANGLEMENT_VALIDATOR
    if _ENTANGLEMENT_VALIDATOR is None:
        _ENTANGLEMENT_VALIDATOR = NoiseBasedEntanglementValidator()
    return _ENTANGLEMENT_VALIDATOR
