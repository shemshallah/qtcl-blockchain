...
# ════════════════════════════════════════════════════════════════════════════════════════════════
# EXPANSION v6.1: ENHANCED PERFORMANCE METRICS & INTEGRATION MONITORING
# ════════════════════════════════════════════════════════════════════════════════════════════════

class ComponentIntegrationMonitor:
    """Monitors health and integration status of all blockchain components."""
    
    def __init__(self):
        self.components = {
            'blockchain': {'status': 'unknown', 'last_check': None, 'error': None},
            'auth': {'status': 'unknown', 'last_check': None, 'error': None},
            'database': {'status': 'unknown', 'last_check': None, 'error': None},
            'quantum': {'status': 'unknown', 'last_check': None, 'error': None},
            'oracle': {'status': 'unknown', 'last_check': None, 'error': None},
            'defi': {'status': 'unknown', 'last_check': None, 'error': None},
            'ledger': {'status': 'unknown', 'last_check': None, 'error': None},
        }
        self.lock = threading.RLock()
        self.health_history: deque = deque(maxlen=1000)
        self.last_integration_check: Optional[datetime] = None
    
    def check_component_health(self, component_name: str, check_fn: Callable) -> Tuple[str, Optional[str]]:
        ...
        with self.lock:
            ...
            self.components[component_name]['last_check'] = datetime.now(timezone.utc).isoformat()
            self.components[component_name]['error'] = error
    ...
    def check_all_components(self) -> Dict[str, Dict[str, Any]]:
        ...
        with self.lock:
            self.last_integration_check = datetime.now(timezone.utc)
    ...
    def get_integration_report(self) -> Dict[str, Any]:
        ...
        with self.lock:
            ...
            return {
                ...
                'component_status': dict(self.components),
                'recent_health_events': list(self.health_history)[-50:],
            }

class PerformanceMetricsCollector:
    """Collects and analyzes performance metrics across all operations."""
    ...
    def record_operation(self, operation_name: str, duration_seconds: float, 
        ...
        with self.lock:
            self.operation_metrics[operation_name].append(duration_seconds)
            
            # Keep only last 10000 entries per operation
            if len(self.operation_metrics[operation_name]) > 10000:
                self.operation_metrics[operation_name] = self.operation_metrics[operation_name][-10000:]
    ...
    def get_operation_stats(self, operation_name: str) -> Optional[Dict[str, float]]:
        ...
        with self.lock:
            durations = self.operation_metrics.get(operation_name, [])
    ...
    def get_slow_operations_report(self, limit: int = 100) -> List[Dict[str, Any]]:
        """Get report of slow operations."""
        with self.lock:
            return list(reversed(list(self.slow_operations)[-limit:]))
    ...
    def get_performance_summary(self) -> Dict[str, Any]:
        ...
        avg_times = [stats['avg_seconds'] for stats in all_stats.values()]
        slowest_op = max(all_stats.items(), key=lambda x: x[1]['max_seconds'])
        
        return {
            'total_operations_tracked': len(all_stats),
            'average_operation_time': sum(avg_times) / len(avg_times),
            'slowest_operation': {
                'name': slowest_op[0],
                'max_seconds': slowest_op[1]['max_seconds'],
                'avg_seconds': slowest_op[1]['avg_seconds'],
            },
            'slow_operations_count': len(self.slow_operations),
            'operation_stats': all_stats,
        }

class EndpointAccessTracker:
    """Tracks access patterns to API endpoints."""
    
    def __init__(self):
        self.endpoint_access: Dict[str, List[Dict[str, Any]]] = defaultdict(list)
        self.lock = threading.RLock()
        self.access_patterns: Dict[str, Dict[str, int]] = defaultdict(lambda: defaultdict(int))
    
    def record_access(self, endpoint: str, method: str, ip_address: str, 
        ...
        with self.lock:
            ...
            # Keep last 1000 accesses per endpoint
            self.endpoint_access[endpoint].append(access_entry)
            if len(self.endpoint_access[endpoint]) > 1000:
                self.endpoint_access[endpoint] = self.endpoint_access[endpoint][-1000:]
            
            # Track patterns
            key = f"{method}:{status_code}"
            self.access_patterns[endpoint][key] += 1
    
    def get_endpoint_stats(self, endpoint: str) -> Optional[Dict[str, Any]]:
        """Get statistics for an endpoint."""
        with self.lock:
            accesses = self.endpoint_access.get(endpoint, [])
            
            if not accesses:
                return None
            
            status_codes = [a['status_code'] for a in accesses]
            response_times = [a['response_time_ms'] for a in accesses]
            
            import statistics
            return {
                'total_accesses': len(accesses),
                'unique_ips': len(set(a['ip_address'] for a in accesses)),
                'avg_response_time_ms': statistics.mean(response_times),
                'min_response_time_ms': min(response_times),
                'max_response_time_ms': max(response_times),
                'status_code_distribution': dict(Counter(status_codes)),
                'most_recent_access': accesses[-1]['timestamp'] if accesses else None,
            }
    ...
    def get_all_endpoint_stats(self) -> Dict[str, Dict[str, Any]]:
        ...
        with self.lock:
            stats = {}
            for endpoint in self.endpoint_access.keys():
                ep_stats = self.get_endpoint_stats(endpoint)
                if ep_stats:
                    stats[endpoint] = ep_stats
            return stats
    
    def detect_suspicious_access_patterns(self) -> List[Dict[str, Any]]:
        """Detect suspicious access patterns (e.g., repeated 401s, 403s)."""
        suspicious = []
        
        with self.lock:
            for endpoint, accesses in self.endpoint_access.items():
                recent = accesses[-100:]  # Last 100 accesses
                
                auth_failures = sum(1 for a in recent if a['status_code'] in [401, 403])
                total_recent = len(recent)
                
                if total_recent > 0 and auth_failures / total_recent > 0.5:
                    suspicious.append({
                        'endpoint': endpoint,
                        'issue': 'High authentication failure rate',
                        'auth_failures': auth_failures,
                        'total_accesses': total_recent,
                        'failure_rate': auth_failures / total_recent,
                    })
        
        return suspicious

# Global monitoring instances
COMPONENT_MONITOR = ComponentIntegrationMonitor()
PERFORMANCE_METRICS = PerformanceMetricsCollector()
ENDPOINT_TRACKER = EndpointAccessTracker()

def get_component_monitor() -> ComponentIntegrationMonitor:
    """Get global component monitor."""
    return COMPONENT_MONITOR

def get_performance_metrics() -> PerformanceMetricsCollector:
    """Get global performance metrics collector."""
    return PERFORMANCE_METRICS

def get_endpoint_tracker() -> EndpointAccessTracker:
    """Get global endpoint access tracker."""
    return ENDPOINT_TRACKER

logger.info("[core_api] ✓ Integration monitoring, performance metrics, and access tracking initialized")
