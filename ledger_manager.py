...
# ════════════════════════════════════════════════════════════════════════════════════════════════════════════
# 🫀 HEARTBEAT INTEGRATION - Register ledger with heartbeat for periodic finalization
# ════════════════════════════════════════════════════════════════════════════════════════════════════════════

def register_ledger_with_heartbeat():
    """Register ledger manager with heartbeat system for periodic updates"""
    try:
        from wsgi_config import HEARTBEAT
        
        if HEARTBEAT is None:
            logger.warning("[Ledger] Heartbeat not available - ledger finalization will be manual")
            return False
        
        def ledger_heartbeat_callback(timestamp):
            """Called every heartbeat pulse - process pending transactions"""
            try:
                # Get ledger instance
                ledger = get_ledger()
                if ledger is None:
                ...
                if hasattr(ledger, 'mempool') and ledger.mempool:
                    pending_count = ledger.mempool.get_pending_count()
                    
                    if pending_count > 0:
                        # Create block from pending transactions
                        if hasattr(ledger, 'block_creator') and ledger.block_creator:
                            block = ledger.block_creator.create_block_from_mempool(ledger.mempool)
                            if block:
                                logger.debug(f"[Ledger-Heartbeat] Block #{block.block_number} created "
                                           f"with {pending_count} transactions")
            except Exception as e:
                logger.warning(f"[Ledger-Heartbeat] Error in heartbeat callback: {e}")
        
        # Register callback
        HEARTBEAT.add_listener(ledger_heartbeat_callback)
        logger.info("[Ledger] ✓ Registered with heartbeat for periodic finalization")
        return True
...
# ════════════════════════════════════════════════════════════════════════════════════════════════
# EXPANSION v6.1: ENHANCED TRANSACTION CONSISTENCY & STATE SYNC
# ════════════════════════════════════════════════════════════════════════════════════════════════

class LedgerConsistencyValidator:
    """Validates ledger consistency and detects state divergence."""
    
    def __init__(self):
        self.consistency_checks: deque = deque(maxlen=5000)
        self.divergence_log: deque = deque(maxlen=1000)
        self.lock = threading.RLock()
        self.last_valid_state: Optional[Dict[str, Any]] = None
    
    def validate_ledger_integrity(self, ledger_state: Dict[str, Any]) -> Tuple[bool, List[str]]:
        """Validate complete ledger integrity."""
        issues = []
        
        try:
            # Check required fields
            required_fields = ['total_transactions', 'total_balance', 'account_count', 'last_block_height']
            for field in required_fields:
                if field not in ledger_state:
                    issues.append(f"Missing required field: {field}")
            
            # Validate numeric constraints
            if ledger_state.get('total_balance', 0) < 0:
                issues.append("Total balance cannot be negative")
            
            if ledger_state.get('account_count', 0) < 0:
                issues.append("Account count cannot be negative")
            
            if ledger_state.get('total_transactions', 0) < 0:
                issues.append("Transaction count cannot be negative")
            
            # Check for consistency with previous state
            if self.last_valid_state:
                prev_height = self.last_valid_state.get('last_block_height', 0)
                curr_height = ledger_state.get('last_block_height', 0)
                
                if curr_height < prev_height:
                    issues.append(f"Block height regression: {curr_height} < {prev_height}")
                
                prev_txs = self.last_valid_state.get('total_transactions', 0)
                curr_txs = ledger_state.get('total_transactions', 0)
                
                if curr_txs < prev_txs:
                    issues.append(f"Transaction count regression: {curr_txs} < {prev_txs}")
            ...
            check_result = {
                ...
                'valid': len(issues) == 0,
                'issues': issues,
                'ledger_height': ledger_state.get('last_block_height'),
                'ledger_balance': ledger_state.get('total_balance'),
            }
            ...
            with self.lock:
                ...
                if len(issues) == 0:
                    self.last_valid_state = json.loads(json.dumps(ledger_state))
                else:
                    for issue in issues:
            ...
            error_msg = f"Consistency check exception: {str(e)}"
    ...
    def get_consistency_report(self) -> Dict[str, Any]:
        ...
        with self.lock:
            checks = list(self.consistency_checks)
            passed = sum(1 for c in checks if c.get('valid', False))
            total = len(checks)
            ...
            return {
                ...
                'divergences_detected': len(self.divergence_log),
                'recent_divergences': list(self.divergence_log)[-20:],
            }

class StateSynchronizer:
    """Manages state synchronization between ledger and blockchain."""
    
    def __init__(self):
        self.sync_operations: deque = deque(maxlen=5000)
        self.pending_syncs: Dict[str, Dict[str, Any]] = {}
        self.lock = threading.RLock()
        self.last_sync_time: Optional[datetime] = None
    
    def queue_sync_operation(self, operation_id: str, ledger_state: Dict[str, Any],
                           blockchain_state: Dict[str, Any]) -> bool:
        """Queue a state synchronization operation."""
        with self.lock:
            if operation_id in self.pending_syncs:
                logger.warning(f"Sync operation {operation_id} already pending")
                return False
            
            self.pending_syncs[operation_id] = {
            ...
            logger.info(f"Queued sync operation: {operation_id}")
            return True
    
    def execute_sync_operation(self, operation_id: str) -> Tuple[bool, Optional[str]]:
        """Execute a state synchronization operation."""
        with self.lock:
            if operation_id not in self.pending_syncs:
                return False, f"Sync operation {operation_id} not found"
            
            sync_op = self.pending_syncs[operation_id]
            
            try:
                # Compare states
                ledger = sync_op['ledger_state']
                blockchain = sync_op['blockchain_state']
                
                ledger_height = ledger.get('last_block_height', 0)
                blockchain_height = blockchain.get('last_block_height', 0)
                
                if ledger_height != blockchain_height:
                    logger.warning(
                        f"Height mismatch: ledger={ledger_height}, blockchain={blockchain_height}"
                    )
                    # Could trigger reorg or state correction here
                
                # Mark as executed
                sync_op['status'] = 'completed'
                sync_op['completed_at'] = datetime.now(timezone.utc).isoformat()
                ...
                self.sync_operations.append({
                    ...
                    'matched': ledger_height == blockchain_height,
                })
                
                self.last_sync_time = datetime.now(timezone.utc)
                logger.info(f"Sync operation {operation_id} completed")
    ...
    def get_sync_status(self, operation_id: str) -> Optional[Dict[str, Any]]:
        ...
        with self.lock:
            return self.pending_syncs.get(operation_id)
    
    def get_sync_report(self) -> Dict[str, Any]:
        """Get comprehensive sync report."""
        with self.lock:
            ops = list(self.sync_operations)
            completed = sum(1 for o in ops if o['status'] == 'completed')
            failed = sum(1 for o in ops if o['status'] == 'failed')
            matched = sum(1 for o in ops if o.get('matched', False))
            
            return {
                'total_sync_operations': len(ops),
                'completed': completed,
                'failed': failed,
                'state_matched': matched,
                'last_sync': self.last_sync_time.isoformat() if self.last_sync_time else None,
                'pending_syncs': len(self.pending_syncs),
                'recent_operations': ops[-20:],
            }

class TransactionReceiptManager:
    """Manages and validates transaction receipts."""
    
    def __init__(self):
        self.receipts: Dict[str, Dict[str, Any]] = {}
        self.receipt_log: deque = deque(maxlen=100000)
        self.lock = threading.RLock()
    
    def create_receipt(self, tx_id: str, block_height: int, tx_index: int,
                      status: str, gas_used: int, output: Any = None) -> Dict[str, Any]:
        """Create a transaction receipt."""
        receipt = {
            'tx_id': tx_id,
            'block_height': block_height,
            'tx_index': tx_index,
            'status': status,  # success, failed, reverted
            'gas_used': gas_used,
            'output': output,
            'timestamp': datetime.now(timezone.utc).isoformat(),
        ...
        logger.debug(f"Receipt created for TX {tx_id}")
        return receipt
    
    def get_receipt(self, tx_id: str) -> Optional[Dict[str, Any]]:
        """Get receipt for a transaction."""
        with self.lock:
            return self.receipts.get(tx_id)
    
    def get_receipts_by_block(self, block_height: int) -> List[Dict[str, Any]]:
        """Get all receipts for a block."""
        with self.lock:
            return [r for r in self.receipts.values() if r['block_height'] == block_height]
    ...
    def get_receipt_stats(self) -> Dict[str, Any]:
        ...
        with self.lock:
            ...
            return {
                ...
                'receipts_in_log': len(self.receipt_log),
            }

# Global instances
CONSISTENCY_VALIDATOR = LedgerConsistencyValidator()
STATE_SYNCHRONIZER = StateSynchronizer()
RECEIPT_MANAGER = TransactionReceiptManager()

def get_consistency_validator() -> LedgerConsistencyValidator:
    """Get global consistency validator."""
    return CONSISTENCY_VALIDATOR

def get_state_synchronizer() -> StateSynchronizer:
    """Get global state synchronizer."""
    return STATE_SYNCHRONIZER

def get_receipt_manager() -> TransactionReceiptManager:
    """Get global receipt manager."""
    return RECEIPT_MANAGER

logger.info("[ledger_manager] ✓ Enhanced consistency validation and state synchronization initialized")
