...
# ════════════════════════════════════════════════════════════════════════════════════════
# MODULE-LEVEL SINGLETONS (PUBLIC - used by mega_command_system, wsgi_config, etc.)
# These are initialized by _init_quantum_singletons() and remain accessible throughout
# the application lifetime. Safe to import and use from other modules.
# ════════════════════════════════════════════════════════════════════════════════════════
...
class UniversalQuantumHeartbeat:
    ...
    def _pulse_loop(self):
        ...
        # ═ MODIFICATION: 15s/30s cycle timing (mega_command_system integration)
...
# ════════════════════════════════════════════════════════════════════════════════════════
# MEGA COMMAND SYSTEM INTEGRATION (Modifications for unified system)
# ════════════════════════════════════════════════════════════════════════════════════════

def get_quantum_systems() -> dict:
    """Get all active quantum systems for mega_command_system integration."""
    return {
        'lattice': LATTICE,
        'heartbeat': HEARTBEAT,
        'lattice_neural_refresh': LATTICE_NEURAL_REFRESH,
...
def get_lattice_metrics() -> dict:
    """Get current lattice metrics for quantum commands."""
    if LATTICE is None:
        return {'status': 'not_initialized'}
    try:
        result = {}
        if hasattr(LATTICE, 'get_w_state'):
            result['w_state'] = LATTICE.get_w_state()
        if hasattr(LATTICE, 'operations_count'):
            result['operations_count'] = LATTICE.operations_count
        result['timestamp'] = time.time()
        return result
    except Exception as e:
        logger.warning(f"[INTEGRATION] get_lattice_metrics error: {e}")
        return {'status': 'error', 'error': str(e)}

def get_noise_bath_state() -> dict:
    """Get current noise bath state."""
    if NOISE_BATH_ENHANCED is None:
        return {'status': 'not_initialized'}
    try:
        if hasattr(NOISE_BATH_ENHANCED, 'get_state'):
            return NOISE_BATH_ENHANCED.get_state()
        return {'status': 'available'}
    except Exception as e:
        logger.warning(f"[INTEGRATION] get_noise_bath_state error: {e}")
        return {'status': 'error', 'error': str(e)}
