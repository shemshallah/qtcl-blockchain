...
# ════════════════════════════════════════════════════════════════════════════════════════════════
# SECTION: COMPREHENSIVE ERROR TRACKING & AUDIT SYSTEM (EXPANSION v6.1)
# ════════════════════════════════════════════════════════════════════════════════════════════════

@dataclass
class ErrorEvent:
    """Immutable error event record for audit trail."""
    timestamp: datetime
    component: str
    error_type: str
    message: str
...
class ErrorTracker:
    """Thread-safe error tracking and audit system."""
    ...
    def record_error(self, component: str, error_type: str, message: str, 
        ...
        with self.lock:
            ...
            if severity == "critical":
                logger.critical(f"CRITICAL ERROR [{component}] {error_type}: {message}")
    ...
    def get_recent_errors(self, limit: int = 100) -> List[Dict[str, Any]]:
        """Get recent errors as list of dicts."""
        with self.lock:
            recent = list(self.errors)[-limit:]
            return [e.to_dict() for e in reversed(recent)]
    
    def get_component_errors(self, component: str) -> List[ErrorEvent]:
        """Get all errors for a component."""
        with self.lock:
            return list(self.component_errors.get(component, []))
    ...
    def get_error_summary(self) -> Dict[str, Any]:
        ...
        with self.lock:
            return {
                ...
                'unique_error_types': dict(self.error_counts),
                'components_with_errors': list(self.component_errors.keys()),
                'error_rates': {k: v for k, v in self.error_counts.items() if v > 0},
                'most_recent_error': self.errors[-1].to_dict() if self.errors else None,
            }

class HealthMonitor:
    """Continuous system health monitoring with thresholds."""
    
    def __init__(self):
        self.lock = threading.RLock()
        self.health_checks: Dict[str, Callable] = {}
        self.last_check: Dict[str, datetime] = {}
        self.health_states: Dict[str, SystemHealth] = {}
    ...
    def perform_check(self, name: str) -> Tuple[SystemHealth, str]:
        """Perform a single health check."""
        with self.lock:
            check_fn = self.health_checks.get(name)
        
        if not check_fn:
            return SystemHealth.OFFLINE, f"No health check registered for {name}"
        ...
        try:
            ...
            with self.lock:
                ...
                self.health_states[name] = status
            
            return status, message
        except Exception as e:
            with self.lock:
                self.health_states[name] = SystemHealth.CRITICAL
            return SystemHealth.CRITICAL, f"{name} check failed: {str(e)}"
    
    def get_system_health(self) -> Tuple[SystemHealth, Dict[str, Any]]:
        """Get overall system health."""
        with self.lock:
            if not self.health_states:
                return SystemHealth.HEALTHY, {}
            
            health_values = [h.value for h in self.health_states.values()]
            worst_health = max(health_values)
            overall_status = [h for h in SystemHealth if h.value == worst_health][0]
            
            return overall_status, dict(self.health_states)
...
def dispatch_command_with_error_tracking(name: str, flags: Dict[str, Any], args: List[str]) -> Dict[str, Any]:
    """Dispatch command with comprehensive error tracking and audit trail."""
    error_tracker = get_error_tracker()
    start_time = time.time()
    
    try:
        result = dispatch_command(name, flags, args)
        elapsed = time.time() - start_time
        logger.debug(f"Command '{name}' executed in {elapsed:.3f}s")
        return result
        ...
        error_tracker.record_error(
            ...
            message=f"Command '{name}' failed: {str(exc)}",
            ...
            severity="critical" if elapsed < 1 else "error"
        )
...
class FallbackProvider:
    ...
    @staticmethod
    def get_default_heartbeat():
        """Return a minimal fallback heartbeat."""
        @dataclass
    ...
    @staticmethod
    def get_default_db_pool():
        """Return a minimal fallback database pool."""
        @dataclass
        class FallbackDBPool:
            def execute(self, query, params=None): return None
            def fetchone(self, query, params=None): return None
            def fetchall(self, query, params=None): return []
        return FallbackDBPool()
    
    @staticmethod
    def get_default_auth_manager():
        """Return a minimal fallback auth manager."""
        @dataclass
        class FallbackAuthManager:
            def verify_token(self, token): return False
            def create_session(self, user_id): return None
        return FallbackAuthManager()

class StateValidator:
    """Validates and enforces state consistency constraints."""
    
    def __init__(self):
        self.constraints: Dict[str, Callable] = {}
        self.violation_log: deque = deque(maxlen=1000)
        self.lock = threading.RLock()
    
    def add_constraint(self, name: str, validator_fn: Callable[[Dict], bool], 
                      error_msg: str = "") -> None:
        """Register a state constraint."""
        with self.lock:
            self.constraints[name] = (validator_fn, error_msg)
    ...
    def validate_state(self, state: Dict[str, Any]) -> Tuple[bool, List[str]]:
        ...
        with self.lock:
            for constraint_name, (validator_fn, error_msg) in self.constraints.items():
                try:
                    if not validator_fn(state):
                        violation = f"{constraint_name}: {error_msg or 'constraint violated'}"
                        violations.append(violation)
                        self.violation_log.append((datetime.now(timezone.utc), violation))
                except Exception as e:
                    violation = f"{constraint_name}: exception during validation: {e}"
                    violations.append(violation)
        
        return len(violations) == 0, violations
    
    def get_violation_history(self) -> List[Tuple[str, str]]:
        """Get recent constraint violations."""
        with self.lock:
            return [(ts.isoformat(), msg) for ts, msg in list(self.violation_log)]

def initialize_globals_with_fallbacks():
    """Initialize globals with graceful fallback for any component failure."""
    global _GS
    
    if _GS is not None and _GS.init_status == InitStatus.INITIALIZED:
        return
    
    error_tracker = get_error_tracker()
    
    try:
        logger.info("Initializing globals with fallback support...")
        initialize_globals()
        _GS = get_globals()
        ...
        for comp_name, getter in critical_components.items():
            try:
                comp = getter()
                if comp is None:
                    logger.warning(f"Component '{comp_name}' returned None, using fallback")
            except Exception as e:
                error_tracker.record_error(
                    component="globals_init",
                    error_type="ComponentInitFailure",
                    message=f"Failed to initialize {comp_name}",
                    stack_trace=traceback.format_exc(),
                    severity="warning"
                )
        
        logger.info("✓ Globals initialized with fallback support")
        return True
    
    except Exception as e:
        error_tracker.record_error(
            component="globals_init",
            error_type="FatalInitError",
            message="Failed to initialize globals",
            stack_trace=traceback.format_exc(),
            severity="critical"
        )
        logger.error("✗ Failed to initialize globals, some functionality may be degraded")
        return False

try:
    initialize_globals_with_fallbacks()
except Exception as e:
