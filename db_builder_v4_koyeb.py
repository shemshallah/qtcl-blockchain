...
if __name__ == "__main__":
    ...
    # ═════════════════════════════════════════════════════════════════════════════════
    # KOYEB PGBOUNCER POOLER DETECTION & CONNECTION CONSTRUCTION
    # ═════════════════════════════════════════════════════════════════════════════════
    
    def construct_connection_url() -> str:
        """
        Hierarchical connection URL builder:
        1. POOLER_* env vars (Koyeb pgBouncer)
        2. DATABASE_URL (standard Supabase/direct)
        3. Fallback localhost
        Returns fully-formed postgresql:// URL
        """
        
        # Koyeb POOLER_ variables (pgBouncer connection pooling)
        pooler_host = os.environ.get('POOLER_HOST')
        pooler_port = os.environ.get('POOLER_PORT', '5432')
        pooler_user = os.environ.get('POOLER_USER')
        pooler_password = os.environ.get('POOLER_PASSWORD')
        pooler_dbname = os.environ.get('POOLER_DBNAME')
        
        if pooler_host and pooler_user and pooler_password and pooler_dbname:
            pg_url = f"postgresql://{pooler_user}:{pooler_password}@{pooler_host}:{pooler_port}/{pooler_dbname}"
            logger.info(f"{CLR.OK}[POOLER] Using Koyeb POOLER_ connection variables{CLR.E}")
            logger.debug(f"  HOST: {pooler_host}:{pooler_port}")
            logger.debug(f"  USER: {pooler_user}")
            logger.debug(f"  DBNAME: {pooler_dbname}")
            return pg_url
        
        # Fallback: DATABASE_URL (standard PostgreSQL/Supabase URL)
        pg_url = os.environ.get('DATABASE_URL')
        if pg_url:
            logger.info(f"{CLR.OK}[DATABASE] Using DATABASE_URL from environment{CLR.E}")
            return pg_url
        
        # Final fallback: localhost (development)
        fallback_url = 'postgresql://postgres:postgres@localhost:5432/qtcl_v4'
        logger.warning(f"{CLR.WARN}[CONNECTION] No POOLER_ or DATABASE_URL vars found. Using localhost fallback.{CLR.E}")
        return fallback_url
    
    # Construct the connection URL
    pg_url = construct_connection_url()
    ...
    logger.info(f"{CLR.HEADER}Connection: Koyeb POOLER | Supabase | Local{CLR.E}")
    logger.info(f"{CLR.HEADER}{'='*80}{CLR.E}\n")
