...
# ════════════════════════════════════════════════════════════════════════════════════════════════
# EXPANSION v6.1: ENHANCED QUANTUM STATE VALIDATION & MEASUREMENT ANALYSIS
# ════════════════════════════════════════════════════════════════════════════════════════════════

class QuantumStateValidator:
    """Validates quantum states and detects measurement anomalies."""
    
    def __init__(self):
        self.state_validations: deque = deque(maxlen=10000)
        self.anomalies: deque = deque(maxlen=1000)
        self.lock = threading.RLock()
    
    def validate_entanglement_metrics(self, ghz_state: Dict[str, Any]) -> Tuple[bool, List[str]]:
        """Validate GHZ entanglement metrics."""
        issues = []
        
        try:
            # Check required metrics
            required = ['entanglement_degree', 'fidelity', 'coherence_time_us']
            for field in required:
                if field not in ghz_state:
                    issues.append(f"Missing metric: {field}")
            
            # Validate ranges
            degree = ghz_state.get('entanglement_degree', 0)
            if not (0 <= degree <= 8):
                issues.append(f"Invalid entanglement degree: {degree} (must be 0-8)")
            
            fidelity = ghz_state.get('fidelity', 0)
            if not (0 <= fidelity <= 1):
                issues.append(f"Invalid fidelity: {fidelity} (must be 0-1)")
            
            coherence = ghz_state.get('coherence_time_us', 0)
            if coherence <= 0:
                issues.append(f"Invalid coherence time: {coherence} (must be > 0)")
            
            # Check for physically plausible degradation
            if fidelity > 0.99 and coherence > 10000:
                # Suspiciously perfect state - might be simulated
                logger.debug(f"Unusually high quality state: fidelity={fidelity}, coherence={coherence}")
            
            with self.lock:
                self.state_validations.append({
                    'timestamp': datetime.now(timezone.utc).isoformat(),
                    'valid': len(issues) == 0,
                    'metrics': ghz_state,
                    'issues': issues,
    ...
    def validate_measurement_outcomes(self, outcomes: Dict[str, int], num_qubits: int) -> Tuple[bool, List[str]]:
        """Validate measurement outcomes distribution."""
        issues = []
        
        try:
            total_measurements = sum(outcomes.values())
            
            if total_measurements == 0:
                issues.append("No measurement outcomes recorded")
            
            # Check for balanced distribution (ideal is 50-50 for binary outcomes)
            if len(outcomes) == 2:
                outcome_list = list(outcomes.values())
                ratio = min(outcome_list) / max(outcome_list) if max(outcome_list) > 0 else 0
                
                if ratio < 0.3:
                    issues.append(f"Measurement outcomes significantly imbalanced: ratio={ratio:.2f}")
            
            # Check for impossible outcomes
            max_outcome = max(int(k) for k in outcomes.keys() if isinstance(k, (int, str)))
            if max_outcome >= (2 ** num_qubits):
                issues.append(f"Outcome {max_outcome} exceeds max for {num_qubits} qubits")
            ...
            error = f"Measurement validation exception: {str(e)}"
            return False, [error]
    
    def get_validation_report(self) -> Dict[str, Any]:
        """Get quantum validation report."""
        with self.lock:
            validations = list(self.state_validations)
...
class EntanglementAnalyzer:
    """Analyzes entanglement patterns and W-state validator coordination."""
    
    def __init__(self, num_validators: int = 5):
        self.num_validators = num_validators
        self.entanglement_history: deque = deque(maxlen=5000)
        self.validator_coordination: Dict[str, List[Dict[str, Any]]] = defaultdict(list)
        self.lock = threading.RLock()
    
    def record_entanglement_event(self, validator_ids: List[str], degree: float,
        ...
        with self.lock:
            ...
            for validator_id in validator_ids:
                self.validator_coordination[validator_id].append(event)
    
    def get_validator_participation_rate(self, validator_id: str) -> float:
        """Get participation rate for a validator."""
        with self.lock:
            if not self.entanglement_history:
                return 0.0
            
            participated = len(self.validator_coordination.get(validator_id, []))
            total = len(self.entanglement_history)
            
            return participated / total if total > 0 else 0.0
    
    def detect_validator_anomalies(self) -> List[Dict[str, Any]]:
        """Detect anomalous validator behavior."""
        anomalies = []
        
        with self.lock:
            participation_rates = {
                vid: self.get_validator_participation_rate(vid)
                for vid in self.validator_coordination.keys()
            }
            
            if participation_rates:
                avg_rate = sum(participation_rates.values()) / len(participation_rates)
                
                for validator_id, rate in participation_rates.items():
                    # Flag validators that are significantly below average
                    ...
                    # Flag validators that are always participating (suspicious)
                    elif rate > 0.99:
                        anomalies.append({
                            'validator_id': validator_id,
                            'issue': 'Suspiciously perfect participation',
                            'rate': rate,
                            'severity': 'low',
                        })
    ...
    def get_entanglement_statistics(self) -> Dict[str, Any]:
        ...
        with self.lock:
            ...
            events = list(self.entanglement_history)
            degrees = [e['degree'] for e in events]
            validator_counts = [e['validator_count'] for e in events]
            
            import statistics
            return {
                'total_events': len(events),
                'avg_entanglement_degree': statistics.mean(degrees),
                'max_entanglement_degree': max(degrees),
                'min_entanglement_degree': min(degrees),
                'avg_validators_per_event': statistics.mean(validator_counts),
                'unique_validators': len(self.validator_coordination),
                'recent_events': events[-20:],
            }

class CoherenceMonitor:
    """Monitors quantum coherence decay and predicts decoherence."""
    
    def __init__(self, warning_threshold_us: float = 100):
        self.warning_threshold_us = warning_threshold_us
        self.coherence_measurements: deque = deque(maxlen=10000)
        self.lock = threading.RLock()
    
    def record_coherence(self, system_id: str, coherence_time_us: float) -> Dict[str, Any]:
        """Record a coherence measurement."""
        entry = {
            'timestamp': datetime.now(timezone.utc).isoformat(),
            'system_id': system_id,
            'coherence_time_us': coherence_time_us,
            'warning_status': 'normal' if coherence_time_us > self.warning_threshold_us else 'degraded',
        }
        
        with self.lock:
            self.coherence_measurements.append(entry)
        
        return entry
    
    def predict_decoherence_time(self, recent_measurements: int = 100) -> Optional[Dict[str, float]]:
        """Predict decoherence time based on trend."""
        with self.lock:
            measurements = list(self.coherence_measurements)[-recent_measurements:]
            
            if len(measurements) < 2:
                return None
            
            coherence_values = [m['coherence_time_us'] for m in measurements]
            ...
            numerator = sum((x_vals[i] - x_mean) * (coherence_values[i] - y_mean) for i in range(n))
            denominator = sum((x_vals[i] - x_mean) ** 2 for i in range(n))
            
            if denominator == 0:
                return None
            
            slope = numerator / denominator  # us per measurement
            
            # Predict when coherence hits 0
            if slope < 0:
                measurements_until_zero = -y_mean / slope
                time_until_zero_minutes = measurements_until_zero / 60  # assuming measurements every minute
                
                return {
                    'slope_us_per_measurement': slope,
                    'current_coherence_us': coherence_values[-1],
                    'estimated_measurements_until_decoherence': measurements_until_zero,
                    'estimated_minutes_until_decoherence': time_until_zero_minutes,
                }
    ...
    def get_coherence_report(self) -> Dict[str, Any]:
        ...
        with self.lock:
            measurements = list(self.coherence_measurements)
            degraded = sum(1 for m in measurements if m['warning_status'] == 'degraded')
            ...
            return {
                ...
                'degraded_measurements': degraded,
                'avg_coherence_us': statistics.mean(coherence_values) if coherence_values else 0,
                'min_coherence_us': min(coherence_values) if coherence_values else 0,
                'max_coherence_us': max(coherence_values) if coherence_values else 0,
                'warning_threshold_us': self.warning_threshold_us,
...
def get_quantum_state_validator() -> QuantumStateValidator:
    """Get global quantum state validator."""
    return QUANTUM_STATE_VALIDATOR

def get_entanglement_analyzer() -> EntanglementAnalyzer:
    """Get global entanglement analyzer."""
    return ENTANGLEMENT_ANALYZER

def get_coherence_monitor() -> CoherenceMonitor:
    """Get global coherence monitor."""
    return COHERENCE_MONITOR

logger.info("[quantum_api] ✓ Quantum state validation, entanglement analysis, and coherence monitoring initialized")
